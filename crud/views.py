...
def delete_user(request, userId):
    try:
        ...
        if request.method == 'POST':
            # This happens when the user clicks "Confirm Delete" on DeleteUser.html
            username = user.username # Store name before deleting for the message
            user.delete()
            messages.success(request, f"User {username} has been successfully removed.")
            return redirect('/user/list')
        
        else:
            # This happens when you click "Delete" on the User List (GET request)
            genderObj = Genders.objects.all()
            data = {
                'user': user,
                'gender': genderObj
